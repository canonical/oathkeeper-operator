...
from charms.identity_platform_admin_ui_operator.v0.admin_ui_service import (
    OathkeeperAdminUIServiceProvider,
)
...
class OathkeeperCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self._admin_ui_relation_name = "oathkeeper-admin-endpoint"
        ...
        self.admin_ui_provider = OathkeeperAdminUIServiceProvider(
    ...
    def _update_admin_ui_relation_data(self, event: RelationEvent) -> None:
        logger.info("Sending endpoint and k8s resources info to Admin UI")
        public_endpoint = (
            self._endpoint
            or f"http://{self.app.name}.{self.model.name}.svc.cluster.local:{OATHKEEPER_API_PORT}"
        )

        public_endpoint = public_endpoint.replace("https", "http")

        self.admin_ui_provider.send_relation_data_for_admin_ui(
            public_endpoint=public_endpoint,
            model=self.model.name,
            rules_configmap="access-rules",
            rules_file="admin_ui_rules.json",
        )
